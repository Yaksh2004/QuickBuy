...
export default function Coupons() {
    ...
    useEffect(() => {
        axios.get("http://localhost:3000/cart/coupons")
        ...
        <div className="flex justify-center items-center w-full">
            <div className="bg-white rounded-md m-4 w-full">
                ...
                <div className="p-6 flex justify-between items-center shadow-[0_-4px_6px_rgba(0,0,0,0.1)]">
                    <button
                        className="text-white font-bold bg-red-400 w-full py-3 rounded-lg hover:bg-red-600 transition cursor-pointer mx-15 disabled:cursor-not-allowed disabled:bg-gray-300"
                        onClick={() => {
                            if (selectedCoupon) {
...
function Coupon({ coupon, itemTotal, selectedCoupon, setSelectedCoupon }) {
    ...
    const discount = eligible
        ? Math.min((coupon.off * itemTotal) / 100, coupon.maxDiscount)
        : 0;

    return (
        <div
            className={`border-b border-gray-200 p-6 ${
                !eligible ? "opacity-50 pointer-events-none" : ""
            }`}
        >
            <div className="flex gap-4 items-center">
                ...
                />
                <div className="font-bold text-red-500 px-4 py-1 border-dashed border border-red-500 w-fit text-sm rounded-sm">
                    {coupon.name}
                </div>
            </div>
            <div className="flex flex-col ml-8 mt-3">
                ...
                <div className="text-xs font-light">
                    Save {coupon.off}% upto ₹{coupon.maxDiscount}
                </div>
                <div className="text-xs font-light">
